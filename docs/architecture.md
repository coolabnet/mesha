...
workspace/
  ...
  adapters/           ← infrastructure readers and normalizers (create when building adapters)
  logs/               ← approved write action logs (create when first write action is logged)
  secrets/            ← never store secrets in committed files (see README)
```

> Note: `adapters/` and `logs/` are not yet created in this workspace. They should be created when you build the first adapter or perform the first approved write action.
...
> Current workspace state: `desired-state/mesh/community-profile/` currently contains only `rollout-policy.yaml`. The `lime-community` file and `desired-state/mesh/node-overrides/` directory must be created before running node onboarding or firmware rollouts. See the playbooks in `docs/playbooks/` for guidance.
